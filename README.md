...
**下载安装**：[Releases 页面](https://github.com/kei-ds/Daily-tasks-and-to-do-lists/releases/latest) → 下载 `DailyWidget-Setup-1.1.0.exe` → 双击运行。目标机器不需要 Node.js。详见[安装](#安装)一节。
...
**锁定（鼠标穿透）**
- 点右上角的锁图标上锁：窗口固定在当前位置，**鼠标点击会穿透到底下的窗口**，
  该点点桌面图标、浏览器、别的程序都不受影响
- 上锁后只有那个锁图标本身可以点，其余内容和缩放手柄一律点不动、拖不动
- 标题栏会显示「已锁定 · 点右边锁图标解除」提示，面板描一圈蓝边，方便看清窗口范围
- 上锁状态会记住，下次启动仍然是锁定的
- 万一找不到锁图标：右键托盘图标 → 取消「锁定」即可解锁
...
去 [Releases](https://github.com/kei-ds/Daily-tasks-and-to-do-lists/releases/latest) 页面下载 `DailyWidget-Setup-1.1.0.exe`，双击运行。

> 从源码 `npm run installer` 构建出来的文件叫 `每日及代办-安装程序-1.1.0.exe`，内容和 Release 里那个完全一样，只是文件名不同——发布到 Release 时特意用了 ASCII 名，因为 GitHub 会把附件名里的中文剥掉，而且部分浏览器下载中文文件名会乱码。
...
产物在 `release\每日及代办-安装程序-1.1.0.exe`。
...
npm run installer    # 生成安装包 release\每日及代办-安装程序-1.1.0.exe（分发用）
...
```bash
bash scripts/lock-test.sh
```

它直接读 Win32 窗口样式里的 `WS_EX_TRANSPARENT` 位——也就是 `setIgnoreMouseEvents`
真正产生的效果——来判断窗口有没有穿透，不依赖应用自报状态。会检查默认不穿透、
锁定状态重启后能恢复、光标移到解锁按钮上时穿透被关掉、移开后恢复。
...
### 锁定时 `forward: true` 不管用，只能自己轮询光标

锁定态的鼠标穿透靠 `win.setIgnoreMouseEvents(true)`。难点在于**解锁按钮本身也得能点**，
所以得知道光标什么时候移到了按钮上。

Electron 文档说 `setIgnoreMouseEvents(true, { forward: true })` 会把 mousemove
转发给渲染进程，按官方文档的写法应该由渲染层监听 mousemove 来判断。**但实测在
`transparent: true` 的窗口上根本收不到任何 mousemove**——挂上计数器、用真实光标反复
移动，事件数始终是 0。

所以改成主进程轮询：渲染进程把解锁按钮的位置上报一次，主进程每 50ms 用
`screen.getCursorScreenPoint()` 判断光标是否落在按钮上，落在上面就把穿透关掉。
